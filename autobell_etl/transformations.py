...
class AutoBellETL:
    ...
    def extract(self):
        ...
        with open(self.car_wash_data, 'r') as file:
            source_car_wash_data_df = pd.read_csv(file,
                                                  index_col=False,
                                                  parse_dates=
                                                  [
    ...
    def transform(self, car_wash_data_df: pd.DataFrame):
        ...
        car_wash_data_df = self._test_period_data_check(car_wash_data_df,
        ...
        car_wash_data_df[self.common_trg_configs.trg_days_col] = (car_wash_data_df[self.common_trg_configs.trg_end_date_col] - car_wash_data_df[self.common_trg_configs.trg_start_date_col]).dt.days
        
        # Create consumption (cubic feet) column for all data df
        car_wash_data_df[self.common_trg_configs.trg_consumption_cubic_col] = car_wash_data_df[self.common_trg_configs.trg_end_meter_cubic_col] - car_wash_data_df[self.common_trg_configs.trg_start_meter_cubic_col]
        
        # Create start and end meter read (gallons) columns for all data df
        car_wash_data_df[self.common_trg_configs.trg_start_meter_gallon_col] = round(car_wash_data_df[self.common_trg_configs.trg_start_meter_cubic_col] * self.common_trg_configs.trg_conversion_factor, 2)
        car_wash_data_df[self.common_trg_configs.trg_end_meter_gallon_col] = round(car_wash_data_df[self.common_trg_configs.trg_end_meter_cubic_col] * self.common_trg_configs.trg_conversion_factor, 2)
        
        # Create consumption (gallons) column for all data df
        car_wash_data_df[self.common_trg_configs.trg_consumption_gallon_col] = round((car_wash_data_df[self.common_trg_configs.trg_end_meter_gallon_col] - car_wash_data_df[self.common_trg_configs.trg_start_meter_gallon_col]), 2)
        
        # Create gallons per car column for all data df
        car_wash_data_df[self.common_trg_configs.trg_gallons_car_col] = round(car_wash_data_df[self.common_trg_configs.trg_consumption_gallon_col] / car_wash_data_df[self.common_trg_configs.trg_cars_col], 2)
        
        #Create Test Period Report
        test_period_report = car_wash_data_df[(car_wash_data_df[self.common_trg_configs.trg_start_date_col] == test_period_start_date) & (car_wash_data_df[self.common_trg_configs.trg_end_date_col] == self.test_period_end_date)].reset_index(drop=True)
    
        
        #Create Pre-Install Report
        
        pre_install_report = car_wash_data_df[(car_wash_data_df[self.common_trg_configs.trg_start_date_col] < test_period_start_date) & (car_wash_data_df[self.common_trg_configs.trg_end_date_col] < test_period_end_date)].reset_index(drop=True)
        ...
        cars_serviced_diff =  test_period_report[self.common_trg_configs.trg_cars_col] - pre_install_cars_mean
        cars_serviced_diff_percentage = round((cars_serviced_diff / pre_install_cars_mean) * 100, 2)
        
        consumption_diff =  test_period_report[self.common_trg_configs.trg_consumption_gallon_col] - pre_install_consumption_gal_mean 
        consumption_diff_percentage = round((consumption_diff / pre_install_consumption_gal_mean) * 100, 2)
        
        gallons_car_diff = test_period_report[self.common_trg_configs.trg_gallons_car_col] - pre_install_gallons_car_mean
        ...
        cash_flow_year_col_vals_contract_rate = self.cash_flow_configs.year_col_list
        ...
        cash_flow_year_col_vals_test_rate = self.cash_flow_configs.year_col_list
        ...
        if self.common_trg_configs.trg_secondary_fluidlytix_cost:
            ...
            cash_flow_year_col_vals_secondary_cost = self.cash_flow_configs.year_col_list
            ...
            return test_period_report, pre_install_report, comparison_report, savings_bep_report_contract_rate, savings_bep_report_test_rate, savings_bep_report_secondary, cash_flow_report_contract_rate, cash_flow_report_test_rate, cash_flow_report_secondary_cost
        else:
            return test_period_report, pre_install_report, comparison_report, savings_bep_report_contract_rate, savings_bep_report_test_rate, cash_flow_report_contract_rate, cash_flow_report_test_rate
    ...
    def _test_period_data_check(self, car_wash_data_df,
        ...
        if test_period_date_check.empty:
            ...
            test_period_data_df[self.common_trg_configs.trg_start_date_col]=pd.to_datetime(test_period_data_df.start_period)
            test_period_data_df[self.common_trg_configs.trg_end_date_col]=pd.to_datetime(test_period_data_df.end_period)
